...
import time, csv, queue, datetime
...
def long_short_order(signal, shifted_time, stock, quantity, price, placed_time):
	app = TestApp("127.0.0.1", 4001, stock_dict[stock].execID)
	...
	if signal == 1:
		...
		#Counter balancing GAT order (this will be a SELL)
		order2=Order()
		order2.action="SELL"
		order2.orderType="MKT"
		order2.totalQuantity=quantity
		order2.conditions = [TimeCondition(isMore=True, time=shifted_time)]
	...
	if signal == -1:
		...
		#Counter balancing GAT order (This will be a BUY)
		order2=Order()
		order2.action="BUY"
		order2.orderType="MKT"
		order2.totalQuantity=quantity
	...
	print(f"Placed market order for {stock}, orderID is {orderid1}. The counter orderID is {orderid2}.")
	...
	print(execdetails)
	'''
	with open(f'logs/contract_fills_{time.strftime("%Y-%m-%d", time.gmtime())}_{stock}.csv', 'a', newline='') as csvfile:
		contractwriter = csv.writer(csvfile, delimiter=',')
		contractwriter.writerow(filldetails.values())

	transaction_list = [stock, price, quantity, shifted_time, orderid2, signal, placed_time]

	#Dump into archive
	with open(f'logs/transactions_{time.strftime("%Y-%m-%d", time.gmtime())}.csv', 'a', newline='') as csvfile5:
		spamwriter5 = csv.writer(csvfile5, delimiter=',')
		spamwriter5.writerow(transaction_list)
